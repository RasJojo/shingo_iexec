    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col px-4 pb-20 pt-10 sm:px-6 lg:px-8">
      <section className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:py-10">
        <div className="space-y-8">
          <Badge className="rounded-full border-red-400/35 bg-red-500/10 px-4 py-1 text-red-700 dark:text-red-100">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              size="lg"
              variant="outline"
              className="h-11 rounded-xl border-slate-300 bg-white/80 dark:border-white/20 dark:bg-slate-950/60"
        ...
        <Card className="glass-panel overflow-hidden border-red-400/20 shadow-glow">
          ...
          <CardContent className="space-y-4 p-6">
            <div className="rounded-lg border border-slate-200 bg-white p-3 font-mono text-xs dark:border-white/10 dark:bg-slate-950/70">
              <div className="mb-2 flex items-center justify-between text-gray-500 dark:text-slate-400">
                <span>{">"} STREAM</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-slate-950/60">
                <p className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-slate-400">24h PnL</p>
                <p className="font-mono text-lg font-semibold text-emerald-600 dark:text-emerald-300">+12.4%</p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-slate-950/60">
                <p className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-slate-400">Signals</p>
                <p className="font-mono text-lg font-semibold text-gray-950 dark:text-white">48</p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-slate-950/60">
                <p className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-slate-400">Winrate</p>
                <p className="font-mono text-lg font-semibold text-red-600 dark:text-red-200">71%</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <section className="my-14 rounded-xl border border-slate-200 bg-white/65 dark:border-white/10 dark:bg-slate-950/65">
        <Marquee>
          {TICKER.map((item) => (
            <div
              key={`${item.pair}-${item.move}`}
              className="flex items-center gap-3 border-r border-slate-200 px-6 py-2 font-mono text-sm dark:border-white/10"
            >
              <span className="text-gray-500 dark:text-slate-400">{item.pair}</span>
              <span className="text-gray-950 dark:text-white">{item.value}</span>
              <span className={item.move.startsWith("+") ? "text-emerald-600 dark:text-emerald-300" : "text-rose-600 dark:text-rose-300"}>
                {item.move}
              </span>
            </div>
          ))}
        ...
        {FEATURES.map((feature) => {
          ...
          return (
            <Card key={feature.title} className="glass-panel">
              <CardHeader>
                <Badge variant="secondary" className="w-fit gap-2 border border-red-400/25 bg-red-500/10 text-red-700 dark:text-red-100">
                  <Icon className="h-3.5 w-3.5" />
                  Core
                </Badge>
                <CardTitle className="font-display text-xl text-gray-950 dark:text-white">{feature.title}</CardTitle>
                <CardDescription className="text-gray-600 dark:text-slate-300">{feature.description}</CardDescription>
              </CardHeader>
            </Card>
          );
        })}
